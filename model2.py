...
wandb.init(project = "Mlops_lesson4_Des_Tree")
...
def test(model, x_test, y_test) -> None:
    ...
    artifact = wandb.Artifact("model_DT_params", type="json")
    artifact.add_file(json_file_path)
    wandb.log_artifact(artifact)

    artifact2 = wandb.Artifact("model_DT_confusion_matrix", type="png")
    artifact2.add_file(image_path)
    wandb.log_artifact(artifact2)

    wandb.summary["DT Accuracy"] = accuracy
    wandb.summary["DT F1 score"] = f1
    wandb.summary["DT AUC-ROC"] = mean_roc_auc
